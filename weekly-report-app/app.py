...
def get_us_week_label(date):
    ...
    first_day = jan1 - datetime.timedelta(days=jan1_weekday)
    delta_days = (date - first_day).days
    week_number = delta_days // 7 + 1
    return f"{year} Week {week_number:02d}"

if 'Store Opening' in df.columns:
    df['Week Label'] = df['Store Opening'].apply(get_us_week_label)
