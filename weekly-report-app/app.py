...
date_cols = [
    col for col in df.columns if any(k in col for k in [
        "⚪ Baseline", "TCO", "Walk", "Turnover",
        "Open to Train", "Store Opening", "Start"
    ])
]
...
try:
    ...
    df['Flag'] = df['Store Opening Delta'].apply(
        lambda x: "Critical" if pd.notna(x) and x >= 5 else ""
    )
except:
...
st.markdown(
    f"<h4><span style='color:red;'><b>{submitted_count}</b></span> form responses have been submitted</h4>",
...
trend_counts = summary_df['Trend'].value_counts().reindex(
    ['🟢 Pulled In', '🔴 Pushed', '🟡 Held', '⚪ Baseline'], fill_value=0
)
