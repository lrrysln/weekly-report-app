...
if submitted and password_input == "1234":
    st.markdown("## 🗓️ Weekly Submission Volume with Expandable Details")
    ...
    for year in years:
        with st.expander(f"📁 {year}"):
            ...
            for _, row in weekly_counts.iterrows():
                week = row['Week Label']
                count = row['Count']
                week_data = year_data[year_data['Week Label'] == week][['Store Number', 'Store Name', 'CPM', 'Prototype', 'Week Label']]
                with st.expander(f"📆 {week} — {count} submission(s)"):
                    st.dataframe(week_data.reset_index(drop=True), use_container_width=True)
else:
    if submitted:
...
# --- Summary for weekly report generation ---
summary_cols = [
...
# --- Placeholders for additional visual or export options ---
# Add call to generate_weekly_summary() if needed elsewhere
